    ...
    public class EntityLittleScareCrow : EntityHumanoid
    {
        ...
        public bool SpawnInterceptor(IBlockAccessor blockAccessor, ref EntityProperties entityProperties, Vec3d spawnPosition, long herdId)
        {
            if (entityProperties.Code.Path.StartsWithFast("hare"))
            {
                ...
                if (distance <= Config.BlockRadiusLittleScarecrowHare)
                {
                    return false;
                }
            }
            ...
            {
                double distance = ServerPos.DistanceTo(spawnPosition);
                if (distance <= Config.BlockRadiusLittleScarecrowRaccoon)
                {
                    return false;
                }
            }
        ...
        public override WorldInteraction[] GetInteractionHelp(IClientWorldAccessor world, EntitySelection es, IClientPlayer player)
        {
            var interactions = ObjectCacheUtil.GetOrCreate(world.Api, "scarecrowInteractions" + EntityId, () =>
            {
                ...
                return new WorldInteraction[] {
                    ...
                    //new()
                    //{
                    //    ActionLangCode = Code.Domain + ":entityhelp-upgrade-lsc",
                    //    MouseButton = EnumMouseButton.Right,
                    //    HotKeyCode = "ctrl",
                    //    Itemstacks = new ItemStack[1] { new(World.GetBlock(new AssetLocation("pumpkin-fruit-4"))) }
                    //},
                    //new()
                    //{
                    //    ActionLangCode = Code.Domain + ":entityhelp-downgrade-lsc",
                    //    MouseButton = EnumMouseButton.Right,
                    //    HotKeyCode = "ctrl",
